...
protected:
    ...
    double lambda_r;     // internal variable, _t -> tumble process; _r -> run process.
...
//    double barriera_p;
    ...
    /** time began tumble */
    double t_t;             // time began tumble
    /** time began run */
    double t_r;             // time began run
...
//    std::uniform_real_distribution<double> std_unifRand; // construct it on (0,1)!
...
public:
    ...
    /** \brief pointer to a engine (if I want to use std::uniform_real_distribution<double>) */
...
//    int initial_condition(double t, double *x0, Funz_C *f);
    ...
    std::array<double,2> X();      // Potrebbe essere una brutta pratica, dacchè viola la classe protetta  !!
    /** return speed bacterium*/
    double V(){return v;};      // Potrebbe essere una brutta pratica, dacchè viola la classe protetta  !!
...
//    double lambda_eat();
    ...
    //    virtual void eat(Funz_C* f_i, double dt);
...
//    friend void Statistical_data_single(E_coli* batterio, double t, ofstream *file_statistic, int tipo_statistica);
